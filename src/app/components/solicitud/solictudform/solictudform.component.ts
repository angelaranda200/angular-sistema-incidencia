...
@Component({
  selector: 'app-solictudform',
  templateUrl: './solictudform.component.html',
  styleUrls: ['./solictudform.component.css']
})
export class SolictudformComponent implements OnInit {
  ...
  postTicket(){
      ...
      (res:any)=>{
        Swal.fire('Ticket Creado',`El ticket con el N° ${res.ticket.idTicket} fue creado con exito`,'success')
    ...
    console.log(this.solicitudForm.controls['adjuntotickets'].value[0].file.type);
  ...
  validarExt():boolean{
    var archivoInput = (<HTMLInputElement>document.getElementById('archivoInput'))
